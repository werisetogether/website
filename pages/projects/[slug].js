...
export default function Index({ project, donate }) {
						...
						<div className="">
							...
							<div className="flex flex-wrap w-full my-10">
								<div className="w-full">
									<div className="sm:text-3xl text-2xl font-medium title-font mb-2 break-all">{title}</div>
							...
							{gallery.map((gallery) => (
								<Link key={gallery.sys.id} gallery={gallery} href={"/gallery/" + gallery.fields.slug}>
									<a className="border-2 border-black hover:bg-gray-100 p-4 rounded-xl text-center">
										<h3 className="text-lg font-medium">{gallery.fields.title}</h3>
									</a>
								</Link>
							))}
						</div>
